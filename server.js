...
async function sendInteraktMessage(id, phone) {
  ...
  console.log("🖼️ Sending Image:", finalImageUrl);
  ...
  console.log("📱 WHATSAPP MESSAGE ID:", data.id);

  return data;
}

/* ==============================
   CREATE USER + AUTO SEND PASS
============================== */
...
app.post("/create", async (req, res) => {
  try {
    ...
    // ⭐ AUTO WHATSAPP SEND
    const whatsapp = await sendInteraktMessage(id, phone);
...
app.get("/user/:id", async (req, res) => {
  ...
  if (!result.rows.length)
    return res.send("<h2>Invalid Pass</h2>");
